...
def main():
    ...
    if args.log_file == "":
        args.log_file = f"logs/test_innov5_quant_step2_cr{args.cr}.log"
    _ = setup_log_redirect(args.log_file)

    device = torch.device(args.device if torch.cuda.is_available() else "cpu")
    bits_list = [int(v) for v in args.bits-list.split(",")] if False else [int(v) for v in args.bits_list.split(",")]
    ...
    csvf = f"logs/test_innov5_quant_step2_cr{args.cr}.csv"
    with open(csvf, "w", encoding="utf-8") as f:
        f.write("bits,nmse,size_mb,size_reduction_ratio\n")

    for b in bits_list:
        # Step2: encoder_fc.weight + SE模块fc.weight
        rules = {
            "encoder_fc.weight": b,
            "encoder_se.fc": b,
            "refine_se.fc": b
        ...
        print(f"[Q{b}-Step2] NMSE={q_nmse:.6f}, Size={q_size/8/1024/1024:.3f} MB, Reduce={(1-q_size/base_size):.4f}")
