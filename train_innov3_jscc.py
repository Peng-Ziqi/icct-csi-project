...
from models.se_csinet_jscc import SECsiNetJSCC
...
def main():
    ...
    if args.log_file == "":
        args.log_file = f"logs/train_innov3_jscc_cr{args.cr}.log"
    ...
    model = SECsiNetJSCC(h=32, w=256, cr=args.cr, se_reduction=8).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    criterion = nn.MSELoss()

    os.makedirs("checkpoints", exist_ok=True)
    os.makedirs("logs", exist_ok=True)
    ckpt = f"checkpoints/se_jscc_cr{args.cr}.pt"
    csv_path = f"logs/train_innov3_jscc_cr{args.cr}.csv"
