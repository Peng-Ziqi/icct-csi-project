...
def main():
    ...
    log_path = os.path.join(args.log_dir, f"train_csinet_plus_cr{args.cr}.log")
    ...
    model = CsiNetPlus(h, w, args.cr).to(args.device)
    ...
    print(f"Start training | CsiNet+ | CR={args.cr} | epochs={args.epochs} | batch={args.batch_size} | lr={args.lr}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_nmse < best_nmse:
            ...
            torch.save(model.state_dict(), f"checkpoints/csinet_plus_cr{args.cr}.pt")
    ...
    plt.title(f"CsiNet+ Val NMSE (CR={args.cr})")
    plt.xlabel("Epoch")
    plt.ylabel("NMSE")
    plt.grid(True)
    plt.savefig(f"plots/val_nmse_csinet_plus_cr{args.cr}.png", dpi=200)

    plt.figure()
    plt.plot(history["train_loss"])
    plt.title(f"CsiNet+ Train Loss (CR={args.cr})")
    plt.xlabel("Epoch")
    plt.ylabel("MSE Loss")
    plt.grid(True)
    plt.savefig(f"plots/train_loss_csinet_plus_cr{args.cr}.png", dpi=200)
