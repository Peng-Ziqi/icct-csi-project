...
def main():
    ...
    log_path = os.path.join(args.log_dir, f"test_csinet_plus_cr{args.cr}.log")
    ...
    ckpt = f"checkpoints/csinet_plus_cr{args.cr}.pt"
    model.load_state_dict(torch.load(ckpt, map_location=args.device, weights_only=True))
    model.eval()

    print(f"Start testing | CsiNet+ | CR={args.cr} | batch={args.batch_size}")
